...
import ru.rkhamatyarov.stockportfolioallocation.client
        .IEXCloudClient;
...
import ru.rkhamatyarov.stockportfolioallocation.exception
        .BusinessException;
...
public final class StockPortfolioCalculationServiceImpl
    ...
    public StockPortfolio calculateStockProportion(
        ...
        List<StockCompany> stockCompanyList = companyStockList
                .getStocks()
                .stream()
                .filter(companyStockVolume ->
                        companyStockVolume.getVolume() > 0)
                .map(companyStockVolume -> StockCompany.builder()
                        ...
                        .latestPrice(iexCloudClient
                                .getLatestPrice(
                                        companyStockVolume.getSymbol()))
                        .volume(companyStockVolume.getVolume())
                        .build())
                .toList();

        List<SectorProportion> sectorProportionList =
                stockCompanyList
                        .stream()
        ...
        Map<String, Integer> sectorAssetValueMap =
                        ...
                        .collect(
                                Collectors.groupingBy(
                                        ...
                                        Collectors.summingInt(
                                                SectorProportion::getAssetValue
                                        )
                                )
                        );
        ...
        Map<String, Double> proportionValueMap =
                sectorAssetValueMap
                        .entrySet()
                        .stream()
                        .collect(
        ...
        Map<String, List<SectorProportion>>
                allocationMap = sectorProportionList
                .stream()
                .collect(
                        Collectors.groupingBy(
                                SectorProportion::getSector
                ...
                .allocations(
                        allocationMap.keySet()
                                ...
                                .map(sector -> SectorProportion.builder()
                                        .sector(sector)
                                        .assetValue(sectorAssetValueMap
                                                .get(sector))
                                        .proportion(proportionValueMap
                                                .get(sector))
                                        .build())
